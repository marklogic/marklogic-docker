...
1. The image must be run in privilaged mode. At the moment if the image isn't run as privilaged many calls that use `sudo` during the startup script will fail due to lack of required permissions as the image will not be able to create a user with the required permissions.
