...
################################################################
if [[ -z $MARKLOGIC_INIT ]] || [[ "$MARKLOGIC_INIT" = "false" ]] || [[ -f /opt/MarkLogic/DOCKER_INIT ]]; then
    echo "MARKLOGIC_INIT is not defined or already initialized, no bootstrap"
else
...
################################################################
if [ -z $MARKLOGIC_JOIN_CLUSTER ] || [[ "$MARKLOGIC_JOIN_CLUSTER" = "false" ]]; then
